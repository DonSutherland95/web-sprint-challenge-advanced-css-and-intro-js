...
function get20s(artists) {
  ...
  for (let i = 0; i < artists.length; i++) {
    let num1 = Number(artists[i].years.split(" ")[0]);
    let num2 = Number(artists[i].years.split(" ")[2]);

    // console.log(num1, num2);

    if (num1 >= 1900 && num2 <= 2000) {
      new_arr.push(artists[i].name);
    }
  }
  return new_arr;
}
...
function lotsOfArt(artistArr) {
  let newArr = [];
  for (let i = 0; i < artistArr.length; i++) {
    if (artistArr[i].paintings > 100) {
      newArr.push(artistArr[i].name);
    }
  }
  return newArr;
}
